...
import { matrixPositionToIndex } from './utils'
export { matrixPositionToIndex }
...
export const cleanupKeymap = () => {
  const filledKeymap = keyboardStore.keymap.map((layer: (string | undefined)[]) => {
    ...
    if (Array.isArray(row)) {
      ...
      row.forEach((keyOrData) => {
        ...
        if (typeof keyOrData === 'string') {
          ...
          if (key.y === undefined || isNaN(key.y)) key.y = currentY
          if (key.x === undefined || isNaN(key.x)) key.x = currentX
          keydata = undefined
          if (!key.w || key.w === 1) {
            currentX++
          } else {
...
export const selectNextKey = () => {
  ...
  selectedKeys.value = new Set([
    Math.min((([...selectedKeys.value][0] || 0) + 1), keyboardStore.keyCount() - 1)
  ])
}
export const selectPrevKey = () => {
  if (selectedKeys.value.size === 0) return
  selectedKeys.value = new Set([Math.max((([...selectedKeys.value][0] || 0) - 1), 0)])
}
        ...
        if (match && match[1]) {
          ...
          switch (key) {
            case 'LT':
              ...
              keylabel.lower = String(params[0]?.action || '')
              keylabel.layer = Number(params[0]?.action || 0)
              keylabel.simple = false
...
export const microcontrollerPinValid = ({
  ...
  return controllers[controller]?.includes(pin) || false
}
