...
import { keyboardStore, selectedKeys, type KeyInfo } from '../store'
...
const keyboardWidth = computed(() => {
  let maxW = 0
  props.keyLayout.forEach((k: KeyInfo) => {
    const width = k.w || 1
...
const rotationOriginX = computed(() => {
  ...
  const firstSelectedKeyIndex = ([...selectedKeys.value][0]) as number
...
const handleKeyClick = ({ keyIndex, event }: { keyIndex: number; event: MouseEvent }) => {
  ...
  if (!event.ctrlKey && !event.metaKey && !event.shiftKey) {
    ...
    if (selectedKeys.value.has(keyIndex)) {
      selectedKeys.value.delete(keyIndex)
    } else {
      selectedKeys.value.add(keyIndex)
    }
  ...
  if (event?.shiftKey && props.mode === 'layout') {
    if (event instanceof MouseEvent) {
      ...
      selectedKeys.value.forEach((keyIndex) => {
        if (keyboardStore.keys[keyIndex]) {
