    ...
    <Popper>
      ...
      <template #content>
        <div class="popover text-left">
          <span>Name</span>
          <input v-if="keyboardStore.layers[index]" v-model="keyboardStore.layers[index]!.name" class="input input-bordered input-sm" />
          <span>Color</span>
          <label class="relative">
            ...
            ></div>
            <input
              v-if="keyboardStore.layers[index]"
          ...
          <div class="mt-2 flex gap-2">
            <div
              ...
              @click="keyboardStore.layers[index] && (keyboardStore.layers[index]!.color = undefined)"
            ></div>
            <div
              class="colorswatch"
              style="background: #0ca508"
              @click="keyboardStore.layers[index] && (keyboardStore.layers[index]!.color = '#0ca508')"
            ></div>
            <div
              class="colorswatch"
              style="background: #259eb9"
              @click="keyboardStore.layers[index] && (keyboardStore.layers[index]!.color = '#259eb9')"
            ></div>
            <div
              class="colorswatch"
              style="background: #f28c18"
              @click="keyboardStore.layers[index] && (keyboardStore.layers[index]!.color = '#f28c18')"
            ></div>
          </div>
        </div>
      </template>
