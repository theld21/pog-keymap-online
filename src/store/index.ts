...
import { matrixPositionToIndex } from '../helpers/utils'
...
export const addToHistory = (keyboard: Keyboard) => {
...
export class Key {
  ...
  setOnKeymap(keyCode: string) {
    ...
    if (keyboardStore.keymap[selectedLayer.value]) {
...
export class Keyboard {
  ...
  getKeymapIndexForKey({ key }: { key: Key }) {
  ...
  getActionForKey({ key, layer }: { key: Key; layer: number }) {
    const navIndex = key.getKeymapIndex()
    if (!this.keymap[layer] || typeof navIndex !== 'number') return 'No layer'
    const keyCode = this.keymap[layer]![navIndex]
  ...
  import({
    ...
    if (true) {
      console.log('importing keyboard features')
...
export const keyboardStore = reactive(new Keyboard())
